...
function loadParentSched() {
  ...
  getMyChildInfo(function(childId, centerId) {
    ...
    supaFetch('madi_schedules?center_id=eq.' + encodeURIComponent(centerId) + '&data->>childId=eq.' + encodeURIComponent(childId) + '&data->>date=gte.' + encodeURIComponent(today) + '&order=id.asc&limit=50', 'GET')
      .then(function(rows) {
...
function _renderParentObsForm(container, childId, centerId) {
  ...
  container.innerHTML =
    ...
    +   '<label for="parentObsCategorySelect" style="font-size:12px;font-weight:600;color:var(--text2);display:block;margin-bottom:5px;">분류</label>'
    +   '<select id="parentObsCategorySelect" aria-label="관찰 기록 분류" class="form-input" style="margin-bottom:0;">'
