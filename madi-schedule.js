...
function renderWeekGrid() {
  ...
  Object.keys(weekByCell).forEach(function(k) {
    weekByCell[k].sort(function(a,b){ return ((a.startTime||a.time)||'') < ((b.startTime||b.time)||'') ? -1 : 1; });
  });
...
function openSchedModal(date, schedId) {
  ...
  overlay.innerHTML = '<div class="sched-modal">'
    ...
    + '<div style="flex:1;"><label class="form-label">치료 시간(분)</label><input class="form-input" type="number" inputmode="numeric" id="schedDuration" placeholder="예:40" min="5" max="180" oninput="autoCalcEndTime()"></div></div>'
...
function openEditSchedModal(id) {
  ...
  overlay.innerHTML = '<div class="sched-modal">'
    ...
    + '<div style="flex:1;"><label class="form-label">치료 시간(분)</label><input class="form-input" type="number" inputmode="numeric" id="editSchedDur" value="' + (s.duration||'') + '" min="5" max="180" placeholder="예:40"></div></div>'
...
function renderWeekGridByChild(weekDates, weekScheds) {
  ...
  Object.keys(childByCell).forEach(function(k) {
    childByCell[k].sort(function(a,b){ return ((a.startTime||a.time)||'') < ((b.startTime||b.time)||'') ? -1 : 1; });
  });
  if (_weekDupOnly) {
