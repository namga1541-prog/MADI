...
function renderMonthGrid() {
  ...
  cells.forEach(function(cell) {
    ...
    html += '<div class="month-cell' + (isToday?' today':'') + (cell.other?' other-month':'') + '" onclick="' + clickFn + '">'
        ...
        if (cells.length === 0) {
          ...
          var items = cells.map(function(s) {
            ...
            return '<div style="font-size:11px;cursor:pointer;line-height:1.3;padding:1px 0;" onclick="openEditSchedModal(\'' + s.id + '\')">'
              ...
              + '<span style="font-weight:700;">' + cname + '</span></div>';
  ...
  if (dayScheds.length === 0) {
    ...
    dayScheds.forEach(function(s) {
      ...
      var closestTime = stdTimes.reduce(function(best, t) {
        return Math.abs(parseInt(t.slice(0,2))*60+parseInt(t.slice(3,5))-rawMm) < Math.abs(parseInt(best.slice(0,2))*60+parseInt(best.slice(3,5))-rawMm) ? t : best;
      }, stdTimes[0]);
      ...
      therapists.forEach(function(t) {
        ...
        if (cell.length===0) {
          ...
          var items = cell.map(function(s) {
            ...
            return '<div style="cursor:pointer;padding:2px 0;" onclick="openEditSchedModal(\'' + s.id + '\')">' + '<span style="font-weight:700;">' + cname + '</span>' + (type?'<br><span style="color:#64748b;font-size:10px;">' + type + '</span>':'') + '</div>';
          }).join('<hr style="border:none;border-top:1px solid #e2e8f0;margin:2px 0;">');
...
function openSchedModal(date, schedId) {
  ...
  overlay.className='sched-modal-overlay'; overlay.id='schedModal';
  ...
  overlay.innerHTML='<div class="sched-modal">'
    ...
    +'<div class="form-row"><div style="flex:1;"><label class="form-label">시작 시간</label><input class="form-input" type="time" id="schedStartTime" step="300" onclick="try{this.showPicker();}catch(e){}" oninput="autoCalcEndTime()"></div>'
    ...
    +'<div id="schedRepeatOpt" style="display:none;"><div class="form-group"><label class="form-label">반복 요일</label><div style="display:flex;gap:7px;margin-top:6px;flex-wrap:wrap;"><button type="button" class="day-chip" data-day="0" onclick="toggleDayChip(this)">일</button><button type="button" class="day-chip" data-day="1" onclick="toggleDayChip(this)">월</button><button type="button" class="day-chip" data-day="2" onclick="toggleDayChip(this)">화</button><button type="button" class="day-chip" data-day="3" onclick="toggleDayChip(this)">수</button><button type="button" class="day-chip" data-day="4" onclick="toggleDayChip(this)">목</button><button type="button" class="day-chip" data-day="5" onclick="toggleDayChip(this)">금</button><button type="button" class="day-chip" data-day="6" onclick="toggleDayChip(this)">토</button></div></div>'
    +'<div class="form-group"><label class="form-label">반복 종료일 <span style="color:var(--mint);font-size:11px;">기본 5년</span></label><input class="form-input" type="date" id="schedRepeatUntil"></div></div>'
  ...
  if(repeat==='weekly'&&until&&until>=date) {
    ...
    while(cur<=end) {
      if(selDays.indexOf(cur.getDay())>-1) {
        entries.push({id:Date.now()+idx*1000,groupId:groupId,childId:childId,date:cur.toISOString().slice(0,10),startTime:startTime,duration:duration,endTime:endTime,note:note,teacher:teacher.trim()});
        idx++;
      }
      cur.setDate(cur.getDate()+1);
    }
  } else {
    entries.push({id:Date.now()+Math.floor(Math.random()*1000),childId:childId,date:date,startTime:startTime,duration:duration,endTime:endTime,note:note,teacher:teacher.trim()});
  }
...
function openEditSchedModal(id) {
  ...
  overlay.innerHTML='<div class="sched-modal">'
    +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><div class="sched-modal-title" style="margin-bottom:0;">📅 일정 상세</div>'
    ...
    +'<div class="form-row"><div style="flex:1;"><label class="form-label">시작 시간</label><input class="form-input" type="time" id="editSchedStart" step="300" value="'+escHtml((s.startTime||s.time)||'')+'" onclick="try{this.showPicker();}catch(e){}"></div>'
...
function goToSessionFromSched(schedId) {
  ...
  setTimeout(function() {
    ...
    if(sel) {
      sel.value=s.childId;
      if(sel._ssInp){ var ch=childDB.find(function(c){ return c.id===s.childId; }); sel._ssInp.value=ch?ch.name+' ('+ch.age+')':''; }
...
function renderWeekGridByChild(weekDates, weekScheds) {
  ...
  childIds.forEach(function(childId){
    ...
    html+='<tr><td style="padding:5px 6px;border:1px solid #e2e8f0;font-weight:700;font-size:11px;color:'+ccolor+';background:'+ccolor+'10;white-space:nowrap;vertical-align:top;position:sticky;left:0;z-index:1;">'+cname+(cage?'<br><span style="font-size:9px;font-weight:400;color:var(--text2);">'+cage+'</span>':'')+'</td>';
    weekDates.forEach(function(w){
      var daySched=weekScheds.filter(function(s){ return s.date===w.str&&s.childId===childId; }).sort(function(a,b){ return ((a.startTime||a.time)||'')<((b.startTime||b.time)||'')?-1:1; });
...
function execSchedDeleteChoice(id) {
  ...
  var ol1=document.getElementById('editSchedOverlay'); var ol2=document.getElementById('delSchedOverlay');
...
function execSchedDelete(id, future) {
  var s=scheduleDB.find(function(x){ return x.id===id; });
  if(!s) return;
  var toDeleteItems=future&&s.groupId?scheduleDB.filter(function(x){ return x.groupId===s.groupId&&x.date>=s.date; }):[s];
  ...
  showToast('🗑️ '+(cn?cn+' ':'')+toDeleteIds.length+'개 일정 삭제됨',{
    undo:function(){ snapshot.forEach(function(item){ scheduleDB.push(item); }); saveSchedule(); renderSchedView(); showToast('↩️ 일정이 복원되었습니다'); }
  });
}
...
function saveEditSched(id) {
  ...
  var endTime='';
  if(start&&dur){ var parts=start.split(':'); var mins=parseInt(parts[0])*60+parseInt(parts[1])+dur; endTime=String(Math.floor(mins/60)%24).padStart(2,'0')+':'+String(mins%60).padStart(2,'0'); }
