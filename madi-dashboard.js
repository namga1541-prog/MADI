...
function renderDashboardTeacher() {
  ...
  if (recentChildren.length === 0) {
    ...
    html += '<div class="dp-children">' + recentChildren.map(function(c){
      ...
      return ''
        + '<div class="dp-child" ' + a11yClick(nm + ' 상세 보기') + ' onclick="openChildDetail(\'' + jsArg(String(c.id)) + '\')">'
...
function renderDashboardAdmin() {
  ...
  if (sortedChanges.length === 0) {
    ...
    sortedChanges.slice(0, 5).forEach(function(c){
      ...
      html += ''
        + '<div class="dp-change-row" ' + a11yClick(nm + ' 상세 보기') + ' onclick="openChildDetail(\'' + jsArg(String(c.id)) + '\')" style="cursor:pointer;">'
