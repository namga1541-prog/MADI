...
- **Fable 5 등 상위 메인모델을 에이전트에 상속·지정하는 것은 대장님이 명시 요청할 때만** (토큰 비용 — 2026-06-10 지시). 기본은 위 표 유지: spawn 시 model 을 반드시 명시(haiku/sonnet/opus)하고, "모델 미지정 = 메인모델 상속"을 기본값으로 쓰지 않는다.
...
- **+ 수정 디프 의미 검수 (커밋 전 필수)**: 에이전트 디프가 ①저장·삭제·undo 경로 ②fire-and-forget 비동기 ③localStorage·캐시 쓰기 ④권한 분기를 건드리면 오케스트레이터가 원문 디프를 직접 검수 — 자동 게이트(lint·smoke·패턴)는 의미 회귀를 못 잡는다(전 게이트 통과 회귀 2건 실증, 2026-06-10). 체크리스트는 [AGENTS.md](./AGENTS.md) §6-1.
- 모든 단계 통과 후 커밋·푸시
