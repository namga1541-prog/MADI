...
### 작업 완료 시 개선 제안 (필수)
- **모든 작업 완료 응답 끝에 반드시 "🔮 추가 개선 제안" 섹션을 두고 최소 2가지 제안**
- 제안 종류 예시: 추가 보강 포인트, 후속 리팩토링, 새로 발견한 디자인/보안/UX 약점, 자동화 기회, 부수 최적화
- 사용자가 명시적으로 "제안 빼" 라고 하지 않는 한 항상 첨부 — 단, 제안 자체가 진짜 가치 있을 때만 (억지 제안 금지, 차라리 1개라도 깊이 있게)
- "다음에 해볼 만한 작업" 보다 **"내가 이번 작업 중 발견했지만 안 한 것"** 위주 — 컨텍스트가 살아있을 때만 가능한 발견
