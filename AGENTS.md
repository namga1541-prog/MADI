...
### 6-1. 수정 디프 의미 검수 (오케스트레이터 필수 — 2026-06-10 규정)

> 근거: Fable 5 출시일 전수감사 수정에서 ESLint·smoke·재발패턴을 **전부 통과한**
> 에이전트 산출물 2건이 의미 회귀였다 — ① 로그아웃 시 fire-and-forget `_oqFlush()`가
> `_oqSave()` 재기록 레이스로 방금 지운 PII 키를 되살림 ② 일정 삭제 undo 가 서버 복원
> 없이 로컬만 되살려 30초 뒤 폴링이 복원분을 재삭제.
> **자동 게이트는 문법과 박제된 패턴만 보증한다. 의미는 오케스트레이터가 디프로 검수한다.**

**커밋 전, 에이전트 디프가 아래 4종을 건드리면 해당 부분을 반드시 직접 git diff 로 원문 검수:**

1. **저장·삭제·복원(undo) 경로** — 서버/인메모리/localStorage 3계층이 같은 결말에 도달하는가?
   (로컬만 복원하면 폴링이 되돌리고, 서버만 지우면 미러가 되살린다.)
2. **fire-and-forget 비동기** — 이후 정리 코드·페이지 전환과의 레이스가 없는가?
   (비동기 콜백이 "정리 완료된 상태"를 다시 쓰는 재기록 레이스 — `_oqSave` 사례)
3. **localStorage·캐시 쓰기** — 새 키가 로그아웃 sweep 목록에 포함되는가? PII 평문 잔존은 없는가?
4. **권한 분기** — 기본값 의미(미설정=허용/차단)가 클라이언트 canDo 와 서버 강제에서 일치하는가?

**대형 수정(5파일+ 또는 위 4종 다수) 시 권장 — 적대적 검증 에이전트 1개 추가 투입:**

```
[적대적 검증] 아래 디프가 깨뜨릴 수 있는 시나리오를 찾아 반박하라.
특히 ① 비동기 레이스 ② 서버/로컬/미러 3계층 불일치 ③ 폴링·재로그인 후 상태를 시뮬레이션할 것.
"문제 없음" 결론을 내려면 각 위험 카테고리별로 왜 안전한지 근거를 제시해야 한다.
[git diff 전문 첨부]
```
