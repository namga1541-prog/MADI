...
| `madi_licenses` | ✅ **테이블+RLS 존재**(인계문서 2026-05-11_B.md line 59 명시) — Edge 화이트리스트(ADMIN_ONLY_TABLES) 등재. admin.html 라이선스 UI가 사용하는 컬럼: `license_key`, `status`, `plan`, `paid_amount`, `paid_at`, `expires_at`, `memo`(+ center_id). ⚠️ **정확한 컬럼 정의는 미기록** — admin 전용 게이트라 비admin 프로브 불가(403), 인계문서도 컬럼 미기재. **admin 계정 SQL Editor에서 아래 실행 후 이 행을 실측 컬럼으로 교체**:<br>`SELECT column_name,data_type FROM information_schema.columns WHERE table_name='madi_licenses' ORDER BY ordinal_position;`<br>(테이블은 존재하므로 CREATE TABLE 마이그레이션 금지 — 충돌 위험) |
