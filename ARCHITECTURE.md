...
## 날짜·시간대 (KST)
- `toKST(d) = new Date(d.getTime() + d.getTimezoneOffset()*60000 + 9h)` — **getTimezoneOffset 을 보정**하므로 **KST 기기에선 `new Date()` == `nowKST()`**. (madi-01.js:33)
...
- 유틸: `ymd(d)`, `getTodayKST()`, `getMonthKST()`, `fmtDateKR()` (madi-01.js), `parseBirth()`/`calcAgeFromBirth()` (madi-10.js).

## 저장 흐름 (데이터 유실 아님)
- `saveSessions()`/`saveChildren()`/`saveSchedule()`/`saveAssess()` (madi-01-app.js): **먼저 localStorage(`cn3_*`)에 미러** → 그 다음 서버 배치 upsert(50개씩, `?on_conflict=id`).
...
## 전역 변수 정의 위치
- `childDB, sessionDB, scheduleDB, assessmentDB, activityDB, iepDB` — madi-01-app.js:134
- `currentUser` — madi-01.js:274 / `_madiToken` — madi-01.js:110
- `noticeDB` — madi-03.js:728 / `_madiApiKey` — madi-03.js:2
