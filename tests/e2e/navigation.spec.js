...
test.describe('네비게이션', () => {
  test('캘린더 탭 — 스케줄 카드 + 내보내기 버튼 표시', async ({ page }) => {
    ...
    await expect(page.locator('button[onclick="openScheduleExportModal()"]')).toBeVisible({ timeout: 10000 });
  });

  test('게시판 탭 — 마디공지/센터공지/고객센터/자료실 4개 탭 표시', async ({ page }) => {
    await page.locator('#sbTab7').click();
    await page.waitForTimeout(2000);
    await expect(page.locator('#bdBtn_global')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('#bdBtn_center')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('#bdBtn_lounge')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('#bdBtn_library')).toBeVisible({ timeout: 10000 });
  });
